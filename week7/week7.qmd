...
Load data
```{r}
magpie <- read_csv(here::here("data", "week7_data", "magpie_obvs.csv"))
tule_elk <- read_csv(here::here("data", "week7_data", "tule_elk_obvs.csv"))

bioclim_dir <- here::here("data", "week7_data", "climate", "wc2.1_2.5m")
...
random_pts <- spatSample(bioclim_crop[["annualMeanTemp"]],
                         ...
                         size = (nrow(magpie) * 2), # sample size- double the species
                         as.points = TRUE) %>% 
  st_as_sf()
...
```{r}
map1 <- tm_shape(bioclim_crop[["annualPrecip"]]) +
...
map2 <- tm_shape(bioclim_crop[["annualMeanTemp"]]) +
